...
渲染规则：`results` 每人一行，`failures` 每人计入数量；`not_recommended` 只出数量和原因，不出现任何个人；"找了 N 轮"里的轮数不含扩张，"看了 N 位"里的人数含扩张。候选人的 candidate_ref、分项分数、PRF 词、公司探测数据、站内筛选降级项、扩张次数都留在文件里，用户视图不出现。报告输出后直接进入步骤 16 的方向选择。
