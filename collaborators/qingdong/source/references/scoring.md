# 隔离评分量表

步骤 11 评分时读。每位候选人单独评分：只依据当前需求版本和这一份详情，各候选人互相独立。

## 硬冲突

只有详情原文明确抵触某条明确的硬性筛选条件或排除信号，才构成硬冲突，必须引用原文。信息缺失、证据弱、能力差距、低分都是评分问题，不是硬冲突。

有硬冲突 = **不符合**；没有 = **符合**。符合只是准入，推荐由总分决定。

## 分项与总分

1. **必须满足匹配分**（0–100）：对照必须满足与硬性筛选条件。90–100 高度匹配，80–89 强匹配，70–79 基本匹配，60–69 弱、存在实质缺口，低于 60 不推荐。
2. **加分项匹配分**（0–100）：需求版本里有加分项才评，否则记"不适用"。分段同上。
3. **风险分**（0–100）：需求版本里有排除信号才评，否则记"不适用"。0 无明确风险，1–29 低，30–59 实质，60–79 高，80–100 严重。
4. **总分** = 必须满足 × 60% + 加分项 × 25% + (100 − 风险) × 15%；不适用的维度剔除后按剩余权重重新归一化，四舍五入到整数。

每条得分注明详情原文依据，依据只从该条 `evidence_section` 指定的区块找。找不到依据的硬条件记 **unknown**：既不给分也不算否定，在结果中逐项列出；计算必须满足匹配分时把 unknown 的条目从分母里剔除，只按有证据（正面或反面）的条目算。详情字段缺失同样记 unknown。需求版本里的 `verify_in_interview` 不参与任何分项，简历上碰巧写了也只作加分项证据。

## 判定口径

- **可推荐** = 符合且总分 ≥ 60。
- **强匹配** = 符合、总分 ≥ 80、必须满足 ≥ 70、风险不适用或 ≤ 30。
- **高分候选人** = 总分 ≥ 70（用于目标公司池扩充，见 SKILL.md 步骤 3.5）。
- **强池** = Top 10 已满且至少 5 位强匹配。
- **扩张门**（用于 SKILL.md 步骤 11.5）= 本轮新评分的候选人中可推荐占比 ≥ 50%，或本轮新增强匹配 ≥ 2；且尚未成强池。只看本轮新人，不看累计。

## Top 10

合并当前需求版本下历轮全部已评分候选人，按总分、必须满足分、candidate_ref 的顺序确定性重排，只展示前 10；退出榜单的候选人评分继续保留。需求版本切换后，所有已评分候选人按新版本重评，Top 10 重建。

原始评分与证据引用保存在下一轮计划的 `decision_basis.candidate_scores`。Builder 校验历史保留、内容寻址的详情引用和需求一致性，并返回确定性的 scores、counts、top10；播报和最终报告使用这些结果，不再由模型重复计算。
