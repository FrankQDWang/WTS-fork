...
| --- | --- |
| 同步日期 | 2026-09-18 |
| 源码来源 | `hewa-domi-app/python/skills/wts/` |
| 技能身份 | `wts` / 猎聘智能寻访 / `$wts` |
| Builder 声明版本 | `0.5.0` |
| 最近一次技能代码提交 | Domi 仓库 `ee5675d`，2026-09-17 |
| 浏览器工作流 | `browser.workflow.v1`，`schema_version=2` |
| 默认交互模式 | `human`；支持显式切回 `direct` |

`source/` 直接是完整技能根目录，内容与上述 Domi 源码一致；不额外套一层 `wts/`，也不带 Python 缓存或运行记录。`0.5.0` 是现有代码中的版本标识，最新变化按日期记录在 [CHANGELOG.md](CHANGELOG.md)，不与本仓库的 `wts-v0-N` 冻结测试版本混用。
...
- [Domi 原子能力说明](DOMI_ATOMIC_CAPABILITIES.md)：技能可使用的浏览器、联网搜索、澄清、文件、执行及其他工具。
...
| 筛选 | 站内筛选、卡片预筛、详情硬筛分层处理；站内筛选失败记录原因并继续，缺失信息保留为 unknown |
...
当前技能不做公开公司研究，目标公司取自 JD 或用户输入；确认稿中的「池子」暂不作为独立检索队列执行。候选人语义评分由 Root 完成，没有独立并发评分器。平台具备联网搜索和子任务能力，不表示 WTS 已启用这些策略。
...
需要配套 Domi 提供 `execute`、文件工具、`request_user_input`、内置浏览器工具，以及当前任务 ID、任务目录、技能目录和 Store 路径。默认拟人模式还要求宿主与插件都支持 `interaction.human.v1`。只复制技能不能为旧客户端补齐这些底座能力。
...
后续修改在 [CHANGELOG.md](CHANGELOG.md) 顶部按日期追加能力变化、原因、底座依赖和需要关注的验证点。本次同步仅做文件一致性与文档静态核对，未执行真实网站任务、运行测试或构建客户端；未包含真实会话、候选人资料或登录数据。
