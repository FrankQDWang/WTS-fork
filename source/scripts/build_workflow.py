...
def read_probe_plan(plan_path: str) -> tuple[dict[str, Any], list[dict[str, str]]]:
    target = Path(plan_path).expanduser().resolve()
    if not target.is_file():
        raise ValueError(f"探测计划文件不存在: {target}")
    if target.stat().st_size > 16 * 1024:
        raise ValueError("探测计划文件不能超过 16 KiB")
    try:
        plan = json.loads(target.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError(f"探测计划不是有效 JSON: {error}") from error
    if not isinstance(plan, dict):
        raise ValueError("探测计划必须是 JSON 对象")
...
def parse_args() -> argparse.Namespace:
    ...
    if args.workflow_type in {"search", "probe"} and not args.plan_file:
        parser.error(f"{args.workflow_type} 工作流必须提供 --plan-file")
    if args.workflow_type in {"search", "probe"}:
        plan_path = Path(args.plan_file).expanduser().resolve()
        expected_plan = (
            expected_search_plan_path(task_work_dir, args.iteration)
            if args.workflow_type == "search"
            else expected_probe_plan_path(task_work_dir, args.iteration)
        )
